...
agregar x (Histograma i t cs) =
  Histograma i t (actualizarElem (indiceCasillero x i t (length cs)) (+ 1) cs)
  where
    indiceCasillero x' i' t' cantidad =
      max (min (floor ((x' - i' + t') / t')) (fromIntegral cantidad - 1)) 0
...
casilleros (Histograma i t cs) =
  zipWith4 Casillero valsMin valsMax cs porcentajes
  where
    valsMin = infinitoNegativo : take (length cs - 1) [i, (i + t) ..]
    valsMax = take (length cs - 1) [i, (i + t) ..] ++ [infinitoPositivo]
    total = sum cs
    porcentajes = if total == 0
                  then replicate (length cs) 0.0  -- Todos 0% si no hay valores
                  else [fromIntegral x * 100 / fromIntegral total | x <- cs]
